...
'''
Generic decorator @protected_route for authentication.

When using this decorator in a function need to ensure that the function passes the FastAPI "Request"
object as "request" in the function parameters.
'''
...
def log_activity_call(log_data):
    def post_log_activity_to_service(data_to_log):
        try:
            # Attempt to post the log data to the activity logging service.
            response = requests.post(url=activity_logging_url, json=data_to_log, timeout=10)
            print(f"Successfully logged event for {data_to_log.get('user_info', {}).get('user_name', 'Unknown')}: {response.text}")
        except Exception as e:
            # Log the exception or handle the failure to log as appropriate.
            print(f"Failed to log activity due to an exception: {e}")
    ...
    try:
        ...
        # Using a thread to perform the logging in a non-blocking way
        Thread(target=post_log_activity_to_service, args=(flattened_data,)).start()

    except Exception as e:
        # Print the traceback to stdout or handle the exception as needed.
        traceback.print_exc(file=sys.stdout)
        # Rethrow or handle the error as appropriate for your application.
        # Example: raise Error(status_code=500, details="Logging failed")

def authenticate_hcmp(token):
    try:
        if token:
        ...
        return {'orgid': org, 'user': user,'orgcode':orgcode}
    except Exception as e:
        traceback.print_exc()
        raise Error(status_code=401, details="Unauthenticated")

def FetchDataFrRedis(token):
    try:
...
def _flatten_urls(menu_svc_url):
    urls = []
...
def protected_route(func):
    ...
    async def check_authentication(*args, **kwargs):
        ...
        redisdata = FetchDataFrRedis(token.lower())
        if redisdata == None:
            raise Error(status_code=401, details="session expired/Token not found in cluster", errorCode="NCDASH0002")

        allowed_urls =  _flatten_urls(redisdata.get("userMenuItems").get("menuServiceURL"))
        ...
        # Bypass URL whitelisting only if testing locally
        if not ('env' in request.headers and request.headers['env'] == 'local'):
            if final_url not in check_authentication.allowed_urls:
        ...
        try:
            ...
            log_data['end_func'] = round(time.time() - start_func, 7)
