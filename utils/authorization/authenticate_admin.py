...
issuer_name = cfg.admin_issuer_name
jwt_key = cfg.jwt_key
...
def log_activity_call(log_data):
    def post_log_activity(flattened_data):
        try:
            response = requests.post(url=activity_logging_url, json=flattened_data, timeout=10)
            print(
                f"Successfully Logged event for {flattened_data.get('user_info', {}).get('user_name', 'Unknown')}: {response.text}")
    ...
    try:
        ...
        # Execute the POST request in a separate thread without waiting for it to complete
        Thread(target=post_log_activity, args=(flattened_data,)).start()
...
def protected_route_admin(func):
    @wraps(func)
    async def check_authentication(*args, **kwargs):
        ...
        if request.cookies.get(admin_token_name):
            print("admin token from cookies")
            token = request.cookies.get(admin_token_name)
            data = authenticate_hcmp(token)
        elif 'env' in request.headers and request.headers['env'] == 'local' and 'token' in request.headers:
            print("admin token from headers")
